...
def plot_model_structure(model):
    ...
    model_structure = visualkeras.layered_view(model, legend=True,show_dimension =True)
    
    fig,ax = plt.subplots(1,1,figsize  = (10,10))
    
    plt.imshow(model_structure)
    ax.set_axis_off()
    plt.savefig("model_structure.png",format = "png")
...
for i in range(len(X_test)):
    ...
    prediction = model.predict(test_image_for_prediction,verbose = 0)
...
ax.bar(x = list(range(2)) ,height = f1_scores_list,width = 0.3,tick_label = ['Car','Bike'])
